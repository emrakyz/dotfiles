...
error_handling() {
        [ -s "${CHANNEL_LIST}" ] || {
                notify-send -i "/home/emre/.config/dunst/youtube.png" "You don't have any channels in 'channels.txt'."
                exit "1"
        }

	grep -q "^.*=https://www.youtube.com/@[[:alnum:]]*/videos$" "${CHANNEL_LIST}" || {
                notify-send -i "/home/emre/.config/dunst/youtube.png" "'channels.txt' formatting is wrong."
                exit "1"
        }

	for pkg in yt-dlp mpv jq; do
                command -v "${pkg}" > "/dev/null" || {
                        notify-send -i "/home/emre/.config/dunst/youtube.png" "${pkg} is not installed. Installing..."
                        "${INSTALLER}" "${pkg}" || {
                                notify-send -i "/home/emre/.config/dunst/youtube.png" "Failed to install ${pkg}."
                                exit "1"
                        }
                }
        done
...
compare_data() {
        ...
        [ -e "${old_data_file}" ] && {
                ...
                [ "${new_videos}" -gt "0" ] &&
		notify-send -i "/home/emre/.config/dunst/youtube.png" -u "critical" "${channel_name} | ${new_videos} videos."
        }
}
